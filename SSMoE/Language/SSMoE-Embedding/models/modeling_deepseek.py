...
class MoEGate(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if self.training and self.alpha > 0.0:
            ...
            aux_loss = None
        return topk_idx, topk_weight, aux_loss, sent_emb, logits
...
class DeepseekMoE(nn.Module):
    ...
    def quantized_rep(self, encoding_indices, embedding, num_embeddings):
        # Encoding
        encoding_indices = encoding_indices.unsqueeze(1)
        encodings = torch.zeros(encoding_indices.shape[0], num_embeddings, device=encoding_indices.device)
        encodings.scatter_(1, encoding_indices, 1)
        
        # Quantize and unflatten
        # encodings = encodings.float()
        # #embedding = embedding.float()
        embedding = embedding.to(encodings.dtype)
        quantized = encodings @ embedding
        #quantized = bnb.matmul_4bit(encodings, embedding, quant_state=embedding.quant_state)
        return quantized
    ...
    def forward(self, hidden_states):
        ...
        topk_idx, topk_weight, aux_loss, sent_emb, logits = self.gate(hidden_states)
        # check if use SVD Router
        if self.use_svd:
            # compute svd router
            if self.svd is None:
                all_vector = None 
                for idx, ex in enumerate(self.experts):
                    _, _, V1 = torch.svd(ex.gate_proj.weight.float().reshape(-1, hidden_dim))
                    similar_v1 = self.gate.weight[idx, :].reshape(1, -1) @ V1 
                    similar_v1 = similar_v1.reshape(-1)
                    _, topv1_idx = torch.topk(similar_v1, k=self.k , dim=0)
                    V1 = V1[:, topv1_idx]
                    _, _, V2 = torch.svd(ex.up_proj.weight.float().reshape(-1, hidden_dim))
                    similar_v2 = self.gate.weight[idx, :].reshape(1, -1) @ V2 
                    similar_v2 = similar_v2.reshape(-1)
                    _, topv2_idx = torch.topk(similar_v2, k=self.k , dim=0)
                    V2 = V2[:, topv2_idx]
                    _, _, V3 = torch.svd(ex.down_proj.weight.T.float().reshape(-1, hidden_dim))
                    similar_v3 = self.gate.weight[idx, :].reshape(1, -1) @ V3 
                    similar_v3 = similar_v3.reshape(-1)
                    _, topv3_idx = torch.topk(similar_v3, k=self.k , dim=0)
                    V3 = V3[:, topv3_idx]
                    #mid_idx = int(hidden_dim/2)
                    S_all = torch.concat([V1.mean(dim=-1).unsqueeze(-1), V2.mean(dim=-1).unsqueeze(-1),V3.mean(dim=-1).unsqueeze(-1)], dim=-1) #dim x 3
                    if all_vector is None:
                        all_vector = S_all.unsqueeze(1)
                    else:
                        all_vector = torch.concat([all_vector, S_all.unsqueeze(1)], dim=1) # dim x n x 3
                all_vector = all_vector.to(hidden_states.dtype)
                self.svd = all_vector.reshape(hidden_dim, -1)
            # get logits of svd router
            quantized_all = hidden_states @ self.svd  
            quantized_all = quantized_all.reshape(batch_size * sequence_length, self.num_experts , -1)
            quantized_all = quantized_all.mean(dim=-1)
            scores_v2 = quantized_all.softmax(dim=-1)
            # get SMoE Router
            scores = logits.softmax(dim=-1)
            # combine SSMoE Router
            scores = scores * (1-self.c) + scores_v2 * self.c
            topk_weight, topk_idx = torch.topk(scores, k=self.top_k, dim=-1, sorted=False)
        else:
            topk_weight, topk_idx = torch.topk(scores, k=self.top_k, dim=-1, sorted=False)
        ...
        sent_emb = logits.view(batch_size, sequence_length, -1)
