...
def _prepare_4d_causal_attention_mask_with_cache_position(
    ...
    if attention_mask is not None and attention_mask.dim() == 4:
        ...
        if attention_mask is not None:
            ...
            padding_mask = causal_mask[:, :, :, :mask_length].cuda() + attention_mask[:, None, None, :].cuda()
...
class Qwen2MoeSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_svd:
            ...
            quantized_all = quantized_all.mean(-1)
            routing_weights_rev2 = F.softmax(quantized_all, dim=1, dtype=torch.float)
            # get SMoE Router
            routing_weights = F.softmax(router_logits, dim=1, dtype=torch.float)
             # combine SSMoE Router
            routing_weights = routing_weights * (1-self.c) + routing_weights_rev2 * self.c
