...
class MoEResidualAttentionBlock(nn.Module):
    ...
    def get_svd(self, hidden_states: torch.Tensor):
        batch_size, sequence_length, hidden_dim = hidden_states.shape
        hidden_states = hidden_states.view(-1, hidden_dim)
        all_vector = None 
        for idx, ex in enumerate(self.experts):
            _, _, V1 = torch.svd(ex.c_fc.weight.float().reshape(-1, hidden_dim))
            V1 = V1.to(self.gate.weight.dtype)
            ...
            V1 = V1[:, topv1_idx]
            _, _, V2 = torch.svd(ex.c_proj.weight.float().reshape(-1, hidden_dim))
            V2 = V2.to(self.gate.weight.dtype)
            ...
            S_all = torch.concat([V1.mean(dim=-1).unsqueeze(-1), V2.mean(dim=-1).unsqueeze(-1)], dim=-1) #dim x 3
    ...
    def forward(self, x: torch.Tensor):
        ...
        #router_logits = (router_logits + quantized_all)/2.0
